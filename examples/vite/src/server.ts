// Vite example for orkify — bundled HTTP server with error triggers
//
// Usage:
//   cd examples/vite && npm install && npm run build
  ...
  switch (path) {
    ...
    case '/reject':
      Promise.resolve().then(() => {
        throw new Error('Unhandled async error from vite bundle');
      });
      break;
  }
}
...
server.listen(PORT, () => {
  console.log(`[vite-example] Worker ${WORKER_ID} listening on port ${PORT}`);
});
